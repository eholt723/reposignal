...
Every decision below was made in service of a single constraint: the project must be fully understandable to a non-technical viewer in 60 seconds or less, running on free-tier infrastructure. That constraint drove scope, data source selection, and architecture choices throughout. GitHub Issues was chosen as the data source specifically because it is public, large-scale, and requires no authentication — removing every setup barrier between a viewer opening the app and seeing a real, populated dashboard. The same pipeline could be pointed at customer support tickets, app store reviews, or any other text-based record stream.
